...
export default async function Checklists({
  ...
  return (
    <>
      <aside className="w-full flex flex-col gap-6 p-6 overflow-hidden overflow-y-auto select-none">
        {symptoms &&
          symptoms.map(({ checklist_name, checklistjson }: ChecklistData) => {
            ...
            return (
              <div key={checklist_name} className="w-full">
                ...
                <div className="pt-10 pb-5 relative flex items-center justify-center dotted-bg">
                  <Link
                    href={"/checklists"}
                    className="absolute left-6 w-8 h-8 flex items-center"
                  >
                      ...
                      return (
                        <tr key={index} className="border">
                          ...
                          <td className="p-5 border align-top uppercase text-sm font-semibold sm:hidden">
                            {isNewHeading && item.heading && (
                              <div className="font-bold py-6 dotted-bg text-xl">
                                {item.heading}
                              </div>
                            )}
                            {item.subheading && (
